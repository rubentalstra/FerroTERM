...
use ferroterm_fhir::r4b::bundle::{Bundle, BundleEntry, BundleEntrySearch};
...
use crate::r4b::metadata::FHIR_VERSION;
use crate::r4b::wire;
use crate::scope::{CACHE_ID_HEADER, cache_id, split_resources};
...
pub async fn versions() -> Response {
    let major_minor: String = FHIR_VERSION
        ...
        .to_owned();
    finish(wire::respond(&Parameters {
        parameter: vec![
            ParametersParameter {
                name: "version".into(),
...
fn run_cache_control(
    ...
    let (parameters, resources) = if body.is_empty() {
        ...
        split_resources(wire::parameters_from_body(headers, body)?)?
    };
    match mode {
        "start" => {
            let id = state.caches().start(resources);
            wire::respond(&Parameters {
        ...
        "end" => {
            ...
            state.caches().end(&id)?;
            wire::respond(&Parameters::default())
        }
...
pub async fn value_set_read(
    ...
    finish(match state.value_set_instance(&id) {
        Some(model) => wire::respond_resource(&render::to_r4b(&model, true)),
...
fn run_value_set_search(state: &AppState, query: &[(String, String)]) -> Result<Response, Failure> {
    ...
    for (id, served_url, served_version) in state.value_set_instances() {
        ...
        entry.push(BundleEntry {
            ...
            resource: Some(Resource::ValueSet(Box::new(render::to_r4b(&model, true)))),

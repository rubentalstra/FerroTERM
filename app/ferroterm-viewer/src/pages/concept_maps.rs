...
/// onto this same route, and `leptos_router` then updates the query without
/// re-running this body (`.claude/rules/leptos-ui.md` §8).
#[component]
#[expect(
    unreachable_pub,
...
fn list_section(
    ...
    view! {
        <section class="mt-8" aria-labelledby="conceptmaps-heading">
            ...
            <Transition fallback=|| {
                view! { <Spinner label="Reading the concept maps" /> }
            }>
                {move || {
                    let params = params.get();
                    let run = run.get();
...
/// `$translate` on `ConceptMap`, so the screen never offers a run the root
/// then refuses (`docs/viewer.md` §5). The answer below it is drawn whenever a
...
fn runner_section(
    ...
    view! {
        <section class="mt-8" aria-labelledby="translate-heading">
            ...
            <Transition fallback=|| {
                view! { <Spinner label="Reading what this root can do" /> }
            }>
                {move || {
                    statement
                        .with(|answered| {
...
fn answer_block(
    ...
    view! {
        <div class="mt-6">
            ...
            <Transition fallback=|| {
                view! { <Spinner label="Running the translation" /> }
            }>
                {move || {
                    translated
                        .with(|answered| {
...
fn count_sentence(found: &SearchSet<PublishedConceptMap>, view: Window) -> String {
    let counted = found.total().map_or_else(
        || format!("This root counted {NOT_DECLARED}."),
        |total| format!("This root counted {total}."),
    );
    format!("{} {counted}", view.summary("concept maps"))
}
...
mod tests {
    ...
    fn the_sentence_states_what_is_drawn_and_what_the_root_counted() {
        let found: SearchSet<PublishedConceptMap> = serde_json::from_str(
            r#"{"total":1,"entry":[{"resource":{"resourceType":"ConceptMap","id":"a"}}]}"#,
        )
        .expect("the fixture is valid JSON");
        assert_eq!(
            count_sentence(&found, window(1, 25, found.matched())),
            "Showing concept maps 1 to 1 of 1. This root counted 1."
        );
    }
}

...
//!
//! A screen names a role, a type step, and a spacing step from this file, and
//! never a palette entry, a raw size, or a `dark:` colour. Two things follow.
//! The palette is measured once, which is how the WCAG 2.2 AA pass in
//! `e2e/tests/it/accessibility.rs` can cover every pairing the viewer draws
...
//!
//! The roles, the six type steps, the four spacing steps, and the density
//! variables are defined in that stylesheet; this file is the vocabulary built
//! on them, and `scripts/checks/viewer-tokens.sh` is what keeps a screen off
//! everything else.
...
pub(crate) const LEAD: &str = "mt-tight text-body text-muted";
...
pub(crate) const TH: &str = "row-y border-b border-line-strong px-default text-left text-small \
                             font-semibold whitespace-nowrap text-muted";

/// A table's cell.
pub(crate) const TD: &str = "row-y border-b border-line px-default align-top";

/// A table cell whose content is one token and must not wrap.
pub(crate) const TD_TIGHT: &str =
    "row-y border-b border-line px-default align-top whitespace-nowrap";

/// The control that submits a form.
pub(crate) const SUBMIT: &str = "state-change inline-flex items-center gap-tight rounded-md \
                                 bg-accent px-default py-tight text-small font-medium text-accent-fg";

/// A control that acts without submitting.
pub(crate) const BUTTON: &str = "state-change inline-flex items-center gap-tight rounded-md border \
                                 border-line px-default py-tight text-small font-medium text-fg \
                                 hover:bg-inset";

/// A control with no chrome until it is pointed at.
pub(crate) const BUTTON_QUIET: &str = "state-change inline-flex items-center gap-tight rounded-md \
                                       px-default py-tight text-small font-medium text-muted \
                                       hover:bg-inset hover:text-fg";

/// A text input.
pub(crate) const INPUT: &str = "w-full rounded-md border border-line-strong bg-raised px-default \
                                py-tight text-body text-fg";
...
pub(crate) const BADGE: &str = "inline-flex items-center gap-tight rounded-full bg-inset px-default py-tight \
                                text-micro font-medium text-muted";

/// A standing mark for something that is working.
pub(crate) const BADGE_OK: &str = "inline-flex items-center gap-tight rounded-full bg-ok-soft px-default \
                                   py-tight text-micro font-medium text-ok-soft-fg";

/// A standing mark for something that failed.
pub(crate) const BADGE_DANGER: &str = "inline-flex items-center gap-tight rounded-full \
                                       bg-danger-soft px-default py-tight text-micro font-medium \
                                       text-danger-soft-fg";

/// A control a screen offers but cannot act on right now.
pub(crate) const BUTTON_DISABLED: &str = "inline-flex items-center gap-tight rounded-md border \

...
| `ValueSet/{id}/$expand` and `ConceptMap/{id}/$translate` answer on every served version, for loaded and persisted instances, and a contradicting `url` is a `400` | `app/ferroterm-server/tests/it/operations.rs::expand_answers_at_the_instance_level_on_every_version`, `::translate_answers_at_the_instance_level_on_every_version`, `::a_persisted_value_set_expands_at_its_instance_id` |

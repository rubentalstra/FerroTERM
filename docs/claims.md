...
| `_summary=text\|data\|false` on read and search and `_summary=count` on search, with `SUBSETTED` on a subset; `_elements` on read; the text view's mandatory elements come from the pinned element table | `app/ferroterm-server/src/elements.rs`; `app/ferroterm-server/tests/it/elements.rs::the_mandatory_elements_are_the_ones_the_pinned_definitions_name`, `::summary_text_on_read_keeps_the_narrative_and_the_mandatory_elements`, `::summary_count_on_search_answers_the_total_and_no_entries`, `::elements_on_read_projects_like_a_search_does` |

...
| `ValueSet/{id}/$expand` and `ConceptMap/{id}/$translate` answer on every served version, for loaded and persisted instances, and a contradicting `url` is a `400`; the capability statement declares the level | `app/ferroterm-server/tests/it/operations.rs::expand_answers_at_the_instance_level_on_every_version`, `::translate_answers_at_the_instance_level_on_every_version`, `::a_persisted_value_set_expands_at_its_instance_id`; `app/ferroterm-server/tests/it/metadata.rs::expand_and_translate_declare_the_instance_level_on_every_version` |
...
| A searchset's `entry.fullUrl` is absolute, from the declared base URL or the authority the request names, plus the version's root | `app/ferroterm-server/src/version/store.rs::base_of`; `app/ferroterm-server/tests/it/code_system.rs::a_search_by_url_and_version_finds_a_loaded_code_system`, `::a_declared_base_url_is_the_one_a_searchset_addresses_resources_at`, `app/ferroterm-server/tests/it/logical_id.rs::full_url` |
...
| A resource written on one served version is left out of another version's search with one `search.mode = outcome` entry, `total` counting the matches, and its instance read on that version is a `406` | `app/ferroterm-server/tests/it/persisted.rs::a_resource_written_on_one_version_leaves_another_version_s_search_answering`, `::a_concept_map_written_as_r4b_leaves_the_r5_search_answering`; `app/ferroterm-server/src/elements.rs::project_bundle` keeps a non-match entry whole |
| A written resource is never converted between FHIR releases | `app/ferroterm-server/src/version/store.rs::rendered`; no specification maps `concept-map-equivalence` to `concept-map-relationship` |

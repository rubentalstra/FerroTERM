...
the citations were last refreshed on 2026-09-23 against the `main` that
shipped after release 0.1.4, with `scripts/checks/versions.sh`,
`scripts/checks/bench-table.sh check`, `scripts/checks/code-systems.sh`,
`scripts/checks/bench-bars.sh`, `scripts/checks/served-bars.sh`, and
...
| `ghcr.io/rubentalstra/ferroterm:0.1.5` is the current image tag | `Cargo.toml` `version = "0.1.5"`, `compose.yaml:36`; enforced by `scripts/checks/versions.sh` and by the tag check in `.github/workflows/release.yml` job `plan` |
...
| The per-system figures (concepts, build, peak build memory, index on disk, resident, per-operation warm p50) | `bench/records/2026-09-23-apple-m2/*.json`, one record per row, rendered by `scripts/checks/bench-table.sh` |
| Warm p50 over 200 HTTP round trips, Apple M2, 17.18 GB, macos/aarch64, FerroTERM 0.1.4, FHIR R4B, 2026-09-22 | The record `machine` and `latency[].warm_requests` fields; the footer is generated by `scripts/checks/bench-table.jq:32-36`, never typed |
| The Snowstorm column reads "not run" for every system | `comparison: null` in every record of the set |
| Taken natively with nothing else running on the machine; the resident and peak-build figures are the server's and the build tool's | `bench/records/2026-09-23-apple-m2/README.md` states the machine state; every record carries `"container": false`, and its `method` field names the instrument behind each figure |
...
| A resource loaded without an `id` keeps the id the server mints, and an authored id outranks a minted one | `app/ferroterm-server/tests/it/logical_id.rs::a_resource_without_an_id_keeps_the_one_the_server_mints`, `::an_authored_id_keeps_the_address_a_minted_id_would_have_taken` |
| A write onto an id a loaded resource is read at answers `409 Conflict` | `app/ferroterm-server/tests/it/logical_id.rs::a_write_onto_a_loaded_id_is_refused` |
| A write whose `url` and `version` another resource of the type carries answers `409 Conflict` with a `duplicate` issue, and a stored duplicate answers from the most recent write | `app/ferroterm-server/tests/it/canonical.rs::a_create_onto_a_persisted_canonical_is_refused`, `::an_update_onto_a_persisted_canonical_is_refused`, `::a_write_onto_a_loaded_canonical_is_refused`, `::the_holder_updates_and_another_version_is_written`, `::a_stored_duplicate_answers_from_the_most_recent_write` |
...
| The seven figure tiles (`$lookup` warm p50, resident, on disk, build time per system) | `bench/records/2026-09-23-apple-m2/*.json`, rendered and checked by `scripts/checks/bench-table.sh check` |
...
| The method (build timed around `ferroterm-build`, peak RSS from `/usr/bin/time`, time to `/health`, the median of five `footprint` readings of the server process on macOS and `ps -o rss=` elsewhere, one cold request then the warm percentiles, nearest-rank) | The `method` field of every record, written by `tools/ferroterm-bench` |
...
| A record states whether its machine was a container, and the repository carries one that a record is only from when it says so | `bench/compose.yaml`; `machine.container` in every record, `false` throughout the published set |
...
| "943 MB resident for the International edition and 1.14 GB for the Dutch one" | `bench/records/2026-09-23-apple-m2/snomed-ct-*.json`, the `rss_warm_bytes` field of each |
...
| Claim | Evidence |
|---|---|
| v0.1.0 ships Linux binaries for `x86_64` and `aarch64` (glibc and musl), an image for `linux/amd64` and `linux/arm64`, and a Compose file, with signed provenance | `.github/workflows/release.yml:194-203` (the four targets), `release-image.yml:156`, `release.yml:179`, and the attest steps |
...
| The design target for point reads is under a millisecond, and a millisecond-scale figure is a performance issue | Stated as a target, not a measurement. Every `$lookup` and `$validate-code` p50 in the current set is under 400 µs, RxNorm's 388 µs included |
| `$lookup` fits a fixed 98 µs plus 4.6 ns per byte of answer across the seven systems, and the bar it is held to has that shape | The `answer_bytes` and `latency` fields of `bench/records/2026-09-23-apple-m2/*.json`; `bench/bars.json` `served_bars`, checked by `scripts/checks/served-bars.sh` in the `bench-bars` CI job |
...
| "559 to 864 MB on disk, 943 MB to 1.14 GB resident, per SNOMED edition" | `bench/records/2026-09-23-apple-m2/snomed-ct-*.json`, the `artifact_bytes` and `rss_warm_bytes` fields, one code system per record |
...
| Claim | Evidence |
|---|---|
| Every operation accepts `GET` with query parameters and `POST` with a `Parameters` resource, at type and instance level | `app/ferroterm-server/tests/it/operations.rs::lookup_by_get_and_post`, `::validate_code_at_type_and_instance_level`, `::subsumes_at_type_and_instance_level`, `::expand_answers_at_the_instance_level_on_every_version`, `::translate_answers_at_the_instance_level_on_every_version` |
| `ValueSet/{id}/$expand`, `ValueSet/{id}/$validate-code` and `ConceptMap/{id}/$translate` answer on every served version, for loaded and persisted instances (including one with no `url`), and a contradicting `url`, version or inline resource is a `400` | `app/ferroterm-server/tests/it/operations.rs::expand_answers_at_the_instance_level_on_every_version`, `::translate_answers_at_the_instance_level_on_every_version`, `::value_set_validate_code_answers_at_the_instance_level_on_every_version`, `::a_persisted_value_set_expands_at_its_instance_id`, `::a_persisted_value_set_without_a_canonical_expands_at_its_instance_id`, `::an_instance_invocation_takes_the_instance_s_own_canonical_and_refuses_another`; `app/ferroterm-server/tests/it/metadata.rs::expand_and_translate_declare_the_instance_level_on_every_version` |
...
| A searchset's `entry.fullUrl` is absolute, from the declared base URL or the authority the request names, plus the version's root, and the bundle carries its `self` link | `app/ferroterm-server/src/version/store.rs::base_of`, `::self_link`; `app/ferroterm-server/tests/it/code_system.rs::a_search_by_url_and_version_finds_a_loaded_code_system`, `::a_declared_base_url_is_the_one_a_searchset_addresses_resources_at`, `app/ferroterm-server/tests/it/elements.rs::a_search_naming_no_elements_is_the_search_it_always_was` |
| `_summary=true\|text\|data\|false` on read and search and `_summary=count` on search, with `SUBSETTED` on a subset; `_elements` on read; the summary view's elements are the pinned element table's `isSummary` flags and the text view's mandatory elements its `min` | `app/ferroterm-server/src/elements.rs`; `app/ferroterm-server/tests/it/elements.rs::the_summary_elements_are_the_ones_the_pinned_definitions_flag`, `::summary_true_on_read_keeps_the_summary_elements`, `::summary_true_on_search_keeps_the_summary_elements_of_every_match`, `::the_mandatory_elements_are_the_ones_the_pinned_definitions_name`, `::summary_text_on_read_keeps_the_narrative_and_the_mandatory_elements`, `::summary_count_on_search_answers_the_total_and_no_entries`, `::elements_on_read_projects_like_a_search_does` |
| A loaded supplement is read at its id and listed by search with `content = supplement` | `app/ferroterm-server/tests/it/code_system.rs::a_loaded_supplement_reads_at_its_id_and_is_listed_by_search` |
...
| The history interaction at the instance and type levels: a `history` `Bundle` newest first, the delete as a `DELETE` entry, `_since`, declared beside `vread` | `app/ferroterm-server/tests/it/history.rs::an_instance_history_lists_every_version_and_the_delete_newest_first`, `::a_type_history_lists_every_resource_of_the_type`, `::since_keeps_the_versions_written_at_or_after_the_instant`, `::the_capability_statement_declares_history_exactly_where_it_declares_vread` |
...
| A resource written on one served version is left out of another version's search with one `search.mode = outcome` entry, `total` counting the matches, and its instance read on that version is a `404` | `app/ferroterm-server/tests/it/persisted.rs::a_resource_written_on_one_version_leaves_another_version_s_search_answering`, `::a_concept_map_written_as_r4b_leaves_the_r5_search_answering`; `app/ferroterm-server/src/elements.rs::apply_bundle` keeps a non-match entry whole |
| A written resource is never converted between FHIR releases | `app/ferroterm-server/src/version/store.rs::rendered`; five `concept-map-equivalence` codes have no `concept-map-relationship` code, and HL7's R4-to-R5 transforms are informative and under development (<https://hl7.org/fhir/extensions/conversions-ConceptMap.html>) |
...
| The `expressions` filter accepts `true` and `false`, and decides whether a value set admits a post-coordinated expression | `crates/fhir-terminology/tests/it/snomed_expression.rs::the_expressions_filter_decides_whether_a_value_set_admits_an_expression` |
| A post-coordinated expression is a code `$lookup`, `$validate-code`, `$subsumes`, and `$closure` answer | `crates/fhir-terminology/tests/it/snomed_expression.rs::a_well_formed_expression_over_defined_concepts_validates`, `::an_expression_is_compared_with_another_expression_both_ways`, `::closure_relates_an_expression_to_the_concepts_a_table_holds`; `app/ferroterm-server/tests/it/snomed_expression.rs::validate_code_accepts_an_expression_on_every_served_version` |
| The Compositional Grammar parser follows the normative ABNF rule for rule and parses the published corpus | `crates/sct-scg/vendor/PROVENANCE.md`; `crates/sct-scg/src/parser.rs`, one function per rule; `crates/sct-scg/tests/it/corpus.rs::every_example_of_the_corpus_parses_and_round_trips`, over 23 vendored examples |
| Expression subsumption is decided over the edition's inferred view, with no description logic classifier. What the server answers `subsumes` or `subsumed-by` is stated by that view; what it answers `not-subsumed` is what that view does not state | `crates/fhir-terminology/src/snomed/expression.rs` (`entails`), which cites SNOMED International's July 2019 withdrawal of normal-form comparison as a subsumption test. Completeness against a classifier is untested here and would need the differential harness over a licensed edition |
...
| The SMART gate accepts every served version's FHIR base under `FERROTERM_BASE_URL` as the token audience, beside `FERROTERM_OIDC_AUDIENCE`, and the admin listener accepts the configured one alone | `app/ferroterm-server/src/smart/mod.rs::Audiences`; `app/ferroterm-server/tests/it/smart.rs::a_token_for_one_version_base_writes_on_every_served_version`, `::an_audience_naming_another_server_is_refused`, `::the_admin_listener_does_not_take_a_version_base_as_its_audience` |
...
- `website/book/src/integrate/fhir-api.md`: "Expansions are flat today; nested
  `contains` is the v0.1.0 milestone."
...
The literal phrase "are v0.1.0" in these lists reads as a version-guard

...
> Notio implements, not a surface you can call today. See the
...
single Lucene index and runs the full edition in about 500 MB. Notio aims to run
...
Notio speaks the FHIR terminology API, so any FHIR client can use it. One such
...
value-set bindings against an external FHIR terminology server. Notio is

...
| `FERROTERM_OIDC_ISSUER` | The OpenID Connect issuer whose access tokens gate the write routes and the admin listener (see [Behind a reverse proxy](reverse-proxy.md#validating-tokens-in-the-server)). The server reads `{issuer}/.well-known/openid-configuration` and the JWKS it names before it binds, and refuses to start when either does not answer. Unset, the server asks for no token and every route answers as it does today. | none (no token is ever asked for) |
| `FERROTERM_OIDC_AUDIENCE` | The `aud` claim every token must carry, so a token minted for another resource server is refused. Unset, any audience is accepted and the issuer check alone bounds the token. | none |
| `FERROTERM_OIDC_ADMIN_SCOPE` | The scope the admin-listener routes require, compared verbatim against the scopes the token grants. No specification governs the admin surface, so the name is yours. | `ferroterm/admin` |

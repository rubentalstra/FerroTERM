...
| `FERROTERM_OIDC_AUDIENCE` | The `aud` claim every token must carry, so a token minted for another resource server is refused. A terminologist's and a reader's tokens both carry it; only the scopes tell the two roles apart. The viewer sends the FHIR base of the version the reader is on as the `aud` of its authorization request, as SMART asks, so an issuer that mints `aud` from that request produces a different audience per version: set this only to a value your issuer mints whatever version a reader signed in from. Unset, any audience is accepted and the issuer check alone bounds the token. | none |

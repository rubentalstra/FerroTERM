...
| `FERROTERM_OIDC_AUDIENCE` | The `aud` claim every token must carry, so a token minted for another resource server is refused. A terminologist's and a reader's tokens both carry it; only the scopes tell the two roles apart. Unset, any audience is accepted and the issuer check alone bounds the token. | none |

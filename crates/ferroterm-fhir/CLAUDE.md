...
`tools/ferroterm-fhir-codegen` from the vendored FHIR packages and is off-limits
...
- To change anything under `src/`, change the emitter or its inputs, then run
  `cargo run -p ferroterm-fhir-codegen -- emit` and commit the regenerated tree.
  `cargo run -p ferroterm-fhir-codegen -- emit --check` is the CI drift check and

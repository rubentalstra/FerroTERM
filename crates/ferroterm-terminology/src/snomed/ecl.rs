...
//! (`ferroterm_ecl::eval::Model`): the closure and the graphs are the
//! artifact's, the concept and description filters are answered from the
//! store and the designation index.

use ferroterm_ecl::ast::{Acceptability, Equality, Sctid, TypedSearchTerm};
use ferroterm_ecl::eval::{ConceptPredicate, DescriptionPredicate, EvalError, Model, term_matches};
use ferroterm_graph::attributes::Attributes;
use ferroterm_graph::identifiers::Identifiers;
use ferroterm_graph::ordinal::Ordinal;
use ferroterm_graph::refsets::RefsetMembers;
use ferroterm_rf2::constants;
use ferroterm_store::record::{self, Designation};
use ferroterm_store::store::{StoreError, Vocabulary};
                ...
                ConceptPredicate::EffectiveTime { operator, values } => {
                    ...
                    for concept in &set {
                        ...
                        let hit = match operator {
                            ferroterm_ecl::ast::Comparison::NotEqual => {
                                values.iter().all(|v| time != *v)
                            }
                            ferroterm_ecl::ast::Comparison::Equal => values.contains(&time),
                            ferroterm_ecl::ast::Comparison::Less => {
                                values.iter().any(|v| time < *v)
                            }
                            ferroterm_ecl::ast::Comparison::LessOrEqual => {
                                values.iter().any(|v| time <= *v)
                            }
                            ferroterm_ecl::ast::Comparison::Greater => {
                                values.iter().any(|v| time > *v)
                            }
                            ferroterm_ecl::ast::Comparison::GreaterOrEqual => {
                                values.iter().any(|v| time >= *v)
                            }
                        };
                        if hit {

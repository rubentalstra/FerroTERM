...
use ferroterm_terminology::operations::{Invocation, OperationError, lookup, validate_code};
...
fn codes(p: &dyn CodeSystemProvider, set: &roaring::RoaringBitmap) -> Vec<String> {
    let mut out: Vec<String> = set
        .iter()
        .filter_map(|i| {
            p.code(ferroterm_terminology::provider::Concept::new(i))
                .expect("reads")
        })
        .collect();
    out.sort();
...
fn the_vendored_hl7_terminology_package_loads() {
    let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tools/ferroterm-fhir-codegen/vendor/hl7.terminology/package");

...
(`addons/*`) are never published. Two `crates/*` members are not published
either, each saying `publish = false` with its `# NOTE: unpublished:` reason
and sitting outside the `CRATES` list while its version moves with the
lockstep line: `terminology-syndication` (the owner's decision, 2026-09-23)
and `sct-scg`, the Compositional Grammar parser, which waits for the owner's
first publish by hand (#652) because the lane cannot create a new crate. Every member is BUSL-1.1 except `rf2`, which

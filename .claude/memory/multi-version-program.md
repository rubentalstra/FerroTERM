...
State on 2026-09-04 (evening): `/r5` reached 500 of 670 (#114's criterion, ticked and clustered on the issue), `/r4b` and `/r4` 479. Merged: #194, #196, #198, #200; open with auto-merge: #201 (R6 ballot under `/r6`, closes #115). The server's per-version maps are two family macros: `version/map_r4.rs` (R4, R4B) and `version/map_r5.rs` (R5, R6 with flavour arms); `r5.rs` is a file like the other versions. Ballot parameters the server does not implement (`manifest`, `filterProperty`, `handle-unclosed-expansion`) are refused as `not-supported` through the inputs' `unsupported` names. The crate line is 0.1.17 on #201. Left in v0.0.9: #116 (XML wire), #168 (a real publish through the lane was dispatched on 2026-09-04 and waits for the owner's environment approval), and the data-gated #12/#13/#15/#18/#19. Fixture artefacts skipped are recorded on #193, #195, #199.

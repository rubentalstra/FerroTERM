...
> (see `docs/architecture.md`). Until `tools/fhir-codegen` exists these
...
1. **Confirm the inputs are the pinned packages.** The vendored FHIR packages
   ...
   `tools/fhir-codegen/vendor/` with a `PROVENANCE.md` each. They are
...
4. **Gate the result:**
   ...
   cargo nextest run -p fhir-types
   ```

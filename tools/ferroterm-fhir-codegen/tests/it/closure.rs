...
use ferroterm_fhir_codegen::fhir::StructureKind;
use ferroterm_fhir_codegen::lower::{Cardinality, Target, TypeKind, VersionModule};
...
fn primitives_keep_element_id_and_extension_and_a_scalar_value() {
    ...
    assert_eq!(
        ...
        Some(&Target::Inline(ferroterm_fhir_codegen::lower::Scalar::Bool))
    );
}
...
fn r5_integer64_is_an_i64_and_expansion_carries_properties() {
    ...
    assert_eq!(
        fields
            ...
            .find(|f| f.name == "value")
            .map(|f| &f.ty.target),
        Some(&Target::Inline(ferroterm_fhir_codegen::lower::Scalar::I64))
    );

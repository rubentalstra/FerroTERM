...
//! RF2 Snapshot through `notio-rf2` and writes the memory-mapped store, the
//! materialized graph, and the description index that `notio-server` opens
//! read-only.
...
/// The command line of `notio-build`.
#[derive(Debug, Parser)]
#[command(name = "notio-build", version, about)]
pub struct Cli {
...
pub enum Error {
    ...
    #[error("the build pipeline is not implemented yet (tracked in rubentalstra/notio#6)")]
    PipelineMissing,
}

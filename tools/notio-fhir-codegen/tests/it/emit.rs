use std::fs;
use std::path::Path;

use notio_fhir_codegen::emit::{EmitError, EmitOptions, emit};

use crate::r4b_dir;

fn options(crate_dir: &Path, check: bool) -> EmitOptions {
    EmitOptions {
        package_dir: r4b_dir(),
        crate_dir: crate_dir.to_path_buf(),
        version_module: String::from("r4b"),
        check,
    }
}

fn tree(root: &Path) -> Vec<(String, String)> {
    let mut files = Vec::new();
    for entry in fs::read_dir(root).expect("dir listable") {
        let path = entry.expect("entry readable").path();
        if path.is_dir() {
            files.extend(tree(&path));
        } else {
            files.push((
                path.to_string_lossy().into_owned(),
                fs::read_to_string(&path).expect("file readable"),
            ));
        }
    }
    files.sort();
    files
}

#[test]
fn emitting_twice_is_byte_identical_and_check_passes() {
    let first = tempfile::tempdir().expect("tempdir");
    let second = tempfile::tempdir().expect("tempdir");
    let report = emit(&options(first.path(), false)).expect("first emit");
    emit(&options(second.path(), false)).expect("second emit");
    assert_eq!(report.types, 133);
    assert_eq!(report.files.len(), 47);
    let a: Vec<String> = tree(&first.path().join("src"))
        .into_iter()
        .map(|(_, c)| c)
        .collect();
    let b: Vec<String> = tree(&second.path().join("src"))
        .into_iter()
        .map(|(_, c)| c)
        .collect();
    assert_eq!(a, b);
    emit(&options(first.path(), true)).expect("check passes on a fresh tree");
}

#[test]
fn every_generated_file_carries_the_banner() {
    let dir = tempfile::tempdir().expect("tempdir");
    emit(&options(dir.path(), false)).expect("emit");
    for (path, content) in tree(&dir.path().join("src")) {
        assert!(
            content.starts_with(
                "// @generated by notio-fhir-codegen from hl7.fhir.r4b.core 4.3.0. DO NOT EDIT.\n"
            ),
            "{path} starts with the banner"
        );
    }
}

#[test]
fn check_reports_edited_missing_and_stale_files() {
    let dir = tempfile::tempdir().expect("tempdir");
    emit(&options(dir.path(), false)).expect("emit");
    let src = dir.path().join("src");
    fs::write(src.join("r4b/coding.rs"), "// edited\n").expect("edit");
    fs::remove_file(src.join("r4b/meta.rs")).expect("remove");
    fs::write(src.join("r4b/stale.rs"), "// stale\n").expect("stale");
    match emit(&options(dir.path(), true)) {
        Err(EmitError::Drift { paths }) => {
            assert_eq!(
                paths,
                vec![
                    "r4b/coding.rs",
                    "r4b/meta.rs (missing)",
                    "r4b/stale.rs (stale)"
                ]
            );
        }
        other => panic!("expected Drift, got {other:?}"),
    }
}

#[test]
fn the_committed_crate_is_in_sync() {
    let crate_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../crates/notio-fhir");
    emit(&options(&crate_dir, true)).expect("the committed notio-fhir crate matches the emitter");
}
